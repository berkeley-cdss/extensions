...
def handle_email_queue(request):
    ...
    logging.info("handle_email_queue called on payload: " + json.dumps(request_json))
    try:
        handle_email.handle_email_queue(request_json=request_json)
        return {"success": True}
    except KnownError as e:
        logging.error("Known Error Occurred: " + str(e) + f" (Request: {request_json})")
        SlackManager().send_error(str(e) + f" (Request: {truncate(request_json)})")
        return {"success": False, "error": str(e)}
    except Exception as e:
        logging.error("Internal Error Occurred: " + str(e) + f" (Request: {request_json})")
...
def handle_flush_gradescope(request):
    request_json = request.get_json()
    logging.info("handle_flush_gradescope called on payload: " + json.dumps(request_json))
    try:
...
def handle_form_submit(request):
    request_json = request.get_json()
    logging.info("handle_form_submit called on payload: " + json.dumps(request_json))
    try:
